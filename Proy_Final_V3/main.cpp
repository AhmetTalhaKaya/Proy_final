...
int main() {
    ...
    time_t tiempoAhora = time(nullptr);
        ...
        tm* fechaHora = localtime(&tiempoAhora);

        // Extrae los componentes de fecha y hora
        int ano = fechaHora->tm_year + 1900;
        int mes = fechaHora->tm_mon + 1;
        int dia = fechaHora->tm_mday;
        int hora = fechaHora->tm_hour;
        int minuto = fechaHora->tm_min;
        int segundo = fechaHora->tm_sec;

        // Muestra la fecha y hora de ejecución
        cout << "Fecha y hora de ejecución: " << dia << "/" << mes << "/" << ano
    ...
    random_device rd;
    mt19937 gen(rd());
    uniform_real_distribution<double> tempDist(-10.0, 45.0);
    uniform_real_distribution<double> humDist(0.0, 100.0);
    uniform_real_distribution<double> precDist(0.0, 50.0);
    uniform_real_distribution<double> luzDist(0.0, 2000.0);
